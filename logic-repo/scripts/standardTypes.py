...
class system_features:
    ...
    def setRf(self, entry, protocolVal):
        self.RfProtocols[entry] = protocolVal
    ...
    def setAllCardTypeApp(self, cardApp):
        for i in range(wl.MAX_CARD_TYPES):
            for j in range(wl.MAX_CARD_APPS):
                self.CardTypesApps[i][j] = cardApp
    ...
    def __setDefaultOpSettings(self):
        ...
        #  Red LED PWM = 60% (6)
        #
        self.OpSettings[wl.LED_OPTIONS_1_INDEX] = 0x5560 | wl.RED_LED | (wl.GREEN_LED << 2)
    ...
    def setMobile(self, option, operation, value):
        if "MASK" == option:
            ...
            self.nfcConfig.set(option, value)
    ...
    def __init__(self):
        ...
        self.KeyPadMap[11] = wl.KP_KEY_HASH_STD
